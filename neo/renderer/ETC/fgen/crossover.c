...
const unsigned char *parent2, unsigned char *child1, unsigned char *child2) {
	...
	if (pop->data_element_size == 32) {
		...
		for (i = 0; i < pop->individual_size_in_bits / 32; i++) {
	...
	if (pop->data_element_size == 64) {
		...
		unsigned int *c1 = (unsigned int *)child1;
		unsigned int *c2 = (unsigned int *)child2;
		for (i = 0; i < pop->individual_size_in_bits / 32; i += 2) {
			int r = fgen_random_2(pop->rng);
			if (r == 0) {
				/* Swap. */
				c1[i] = p2[i];
	...
	for (i = 0; i < pop->individual_size_in_bits; i += pop->data_element_size) {
		int r = fgen_random_2(pop->rng);
		if (r == 0) {
			/* Swap this element. */
...
const unsigned char *parent2, unsigned char *child1, unsigned char *child2) {
	...
	for (i = 0; i < pop->permutation_size; i++)
		skip[i] = 0;
