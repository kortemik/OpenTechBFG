...
namespace texgen {
	...
	static bool generate_mipmap_level(Image *source_image, int divider, Image *dest_image) {
		...
		if (source_image->bits_per_component == 8 && source_image->is_signed) {
			...
			if (source_image->is_half_float || source_image->bits_per_component == 16 || source_image->is_signed) {
				return false;
			}
	...
	bool generate_mipmap_level_from_original(Image *source_image, int level, Image *dest_image) {
		int divider = 1 << level;
		return generate_mipmap_level(source_image, divider, dest_image);
	}

	bool generate_mipmap_level_from_previous_level(Image *source_image, Image *dest_image) {
		return generate_mipmap_level(source_image, 2, dest_image);
	}
