...
namespace texgen {
	...
	static bool half_float_table_init = false;
	static float half_float_table[65536];

	static bool gamma_corrected_half_float_table_init = false;
	static float gamma_corrected_half_float_table[65536];

	static bool normalized_float_table_init = false;
	static float normalized_float_table[256];
...
#define compare_pixel_xy_with_alpha(valx, valy) \
	...
	void calculate_normalized_float_table() {
		if (normalized_float_table_init)
			return;
		for (int i = 0; i < 256; i++) {
	...
	double compare_block_4x4_rgba_half_float(unsigned int *image_buffer, BlockUserData *user_data, FILE* error_print) {
		...
		if (isnan(error) && error_print) {
			fprintf(error_print, "Error -- unexpected NaN in half float block.\n");
			//		exit(1);
		}
		return (double)1 / error;
	}
	...
	void calculate_gamma_corrected_half_float_table() {
		if (gamma_corrected_half_float_table_init)
			return;
		for (int i = 0; i < 65536; i++) {
			uint16_t h[1];
			float f[1];
	...
	const float *get_gamma_corrected_half_float_table() {
		if (!gamma_corrected_half_float_table_init)
			return NULL;
		return gamma_corrected_half_float_table;
	}
