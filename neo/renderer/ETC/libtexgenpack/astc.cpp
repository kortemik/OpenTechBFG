...
namespace texgen {
	...
	bool convert_astc_texture_to_image(const Options *opt, Texture *texture, Image *image) {
		...
		if (!save_astc_file(texture, astc_filename))
			return false;
		...
		free(s);
		// Load the .ktx file.
		bool ret = load_image(opt, ktx_filename, FILE_TYPE_KTX, image);
	...
	bool decompress_astc_file(const Options *opt, const char *filename, Image *image) {
		char *tmp_filename = tmpnam(NULL);
		char *ktx_filename = (char *)alloca(strlen(tmp_filename) + 5);
		strcpy(ktx_filename, tmp_filename);
		strcat(ktx_filename, ".ktx");
	...
	bool compress_image_to_astc_texture(const Options *opt, Image *image, int texture_type, Texture *texture) {
		...
		if (!save_png_file(image, png_filename))
			return false;
		// Create temporary .astc filename
		tmp_filename = tmpnam(NULL);
		char *astc_filename = (char *)alloca(strlen(tmp_filename) + 6);
		...
		const char *astcenc_speed_option;
		switch (opt->speed) {
		case SPEED_ULTRA :
		...
		bool ret = load_astc_file(astc_filename, texture);
		remove(astc_filename);
		return ret;
	}
