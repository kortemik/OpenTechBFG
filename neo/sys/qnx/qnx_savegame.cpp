...
//XXX idCVar savegame_winInduceDelay( "savegame_winInduceDelay", "0", CVAR_INTEGER, "on windows, this is a delay induced before any file operation occurs" );
...
========================
*/
idLocalUserQnx * GetLocalUserFromSaveParms( const saveGameThreadArgs_t & data ) {
	if ( ( data.saveLoadParms != NULL) && ( data.saveLoadParms->inputDeviceId >= 0 ) ) {
		...
		if ( user != NULL ) {
			idLocalUserQnx * userQnx = static_cast< idLocalUserQnx * >( user );
			if ( userQnx != NULL && data.saveLoadParms->userId == idStr::Hash( userQnx->GetGamerTag() ) ) {
				return userQnx;
			}
		}
	}
...
int idSaveGameThread::Save() {
	idLocalUserQnx * user = GetLocalUserFromSaveParms( data );
	if ( user == NULL ) {
	...
	// Save the raw files.
	for ( int i = 0; i < callback->files.Num() && ret == EOK && !callback->cancelled; i++ ) {
		idFile_SaveGame * file = callback->files[i];
		...
		if ( outputFile == NULL ) {
			idLib::Warning( "[%s]: Couldn't open file for writing, %s. Error = %08x", __FUNCTION__, tempFileName.c_str(), errno );
			file->error = true;
		...
		if ( ( file->type & SAVEGAMEFILE_PIPELINED ) != 0 ) {
			...
			while ( inputFile->NextWriteBlock( & block ) ) {
				if ( (size_t)outputFile->Write( block.data, block.bytes ) != block.bytes ) {
					idLib::Warning( "[%s]: Write failed. Error = %08x", __FUNCTION__, errno );
					file->error = true;
					callback->errorCode = SAVEGAME_E_INSUFFICIENT_ROOM;
					ret = -1;
					break;
	...
	if ( fileSystem->IsFolder( saveFolder, "fs_savePath" ) == FOLDER_YES ) {
		...
		for ( int i = 0; i < fileList.Num() && !callback->cancelled; i++ ) {
			...
			if ( file != NULL ) {
				...
				struct stat st;
				if ( stat( file->GetFullPath(), &st ) == 0 ) {
					details->date = st.st_mtime;
				}
			} else {
				details->damaged = true;
			}
...
int idSaveGameThread::Run() {
	...
	try {
		...
		/* XXX if ( savegame_winInduceDelay.GetInteger() > 0 ) {
